...
func TestLeaderboard_Sorting(t *testing.T) {
	...
	err = engine.SubmitAnswer(ctx, run.ID, 1, 0, 1) // неправильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 2, 0, 0) // правильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 3, 0, 0) // правильно
	require.NoError(t, err)
	...
	err = engine.SubmitAnswer(ctx, run.ID, 1, 1, 1) // неправильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 2, 1, 0) // правильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 3, 1, 1) // неправильно
	require.NoError(t, err)

	// Q3
	<-events

	err = engine.SubmitAnswer(ctx, run.ID, 1, 2, 1) // неправильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 2, 2, 0) // правильно
	require.NoError(t, err)
	err = engine.SubmitAnswer(ctx, run.ID, 3, 2, 1) // неправильно
	require.NoError(t, err)
...
func TestLeaderboard_SameScore_SortByTime(t *testing.T) {
	...
	err = engine.SubmitAnswer(ctx, run.ID, 2, 0, 0)
	require.NoError(t, err)
	time.Sleep(10 * time.Millisecond)
	// slow отвечает вторым
	err = engine.SubmitAnswer(ctx, run.ID, 1, 0, 0)
	require.NoError(t, err)
...
func TestQuizWithTextAnswers_FullFlow(t *testing.T) {
	...
	err = engine.SubmitAnswerByLetter(ctx, run.ID, 1, "B") // правильно
	require.NoError(t, err)
	err = engine.SubmitAnswerByLetter(ctx, run.ID, 2, "A") // неправильно
	require.NoError(t, err)

	// Q2
	<-events

	err = engine.SubmitAnswerByLetter(ctx, run.ID, 1, "A") // правильно
	require.NoError(t, err)
	err = engine.SubmitAnswerByLetter(ctx, run.ID, 2, "A") // правильно
	require.NoError(t, err)
